%fsrsolver.m
%This code uses the discrete resonance method to calculate the exact FSRs
%of a resonance frequency. If the input wavelength does not correspond to a
%resonance, fsrsolver will find the nearest resonance. FSRs given in GHz.
%function [FSR1 FSR2 SZ] = dnusolver_check(width, rOut, clambda, xs, nEffGuess)
function [fsr1 fsr2 wavelengths gammas SZ] = fsrsolver(width, rOut, clambda, xs, nEffGuess)
tic
...
lambdas = blambda:(ulambda-blambda)/2:ulambda;
...
OPTS.NMODES_CALC = 1;                 %Number of modes you want to calculate
...
omegas = interp1(xgamma,pomega,gammas);
wavelengths = 2*pi*c./omegas;

%% Find the FSR of the closest values
[val, idx] = min(abs(omegas - 2*pi./(clambda*um)*c));
fsr1 = (omegas(idx) - omegas(idx-1))/(2*pi*1e9);
fsr2 = (omegas(idx+1) - omegas(idx))/(2*pi*1e9);
