...
nlyrs = ones(3, 5);
nlyrs(3,:) = nSiO2;
nlyrs(2,1) = nSiO2; nlyrs(2,3) = nSiO2; nlyrs(2,5) = nSiO2;
nlyrs(2,2) = nSi; nlyrs(2,4) = nSi;
nlyrs(1,:) = nSiO2;


%Layer thickness and widths
wWg1 = .45;            %waveguide width .45
wWg2 = .35;            %Waveguide2 width .3
gap = .3;
...
dlyrsx = [side wWg1 gap wWg2 side];                     
dlyrsy = [bot Sithick top];               %microns tall

%Run modesolver
dxy = [0.01, 0.01];                   %Discretization
k0 = 2*pi/1.55;
OPTS.mu_guess = 3*2*pi/1.55;          %guess of propagation constant beta;
...
figure; imagesc(N.y, N.x, (N.n).'); set(gca, 'YDir', 'normal'); colorbar;
