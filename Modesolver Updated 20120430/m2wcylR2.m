...
function [mu, F, V, D, Pr, Pz, Rmtx] = m2wcylR2(S, nu_index, mu_guess, OPTS, nmodes, dPML, sigmaPML)
...
%    mu = +X(:,2) + i*X(:,1)*asgn;  % [MPREM] swap columns wrt top part of this "if"-statement.
    mu = +X(:,2) - 1i*X(:,1)*asgn;  % keeping wI < 0, swap columns wrt top part of this "if"-statement.
    ...
    mu = -X(:,2) + 1i*X(:,1)*asgn;  % swap columns wrt top part of this "if"-statement.
end
