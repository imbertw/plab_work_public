...
function [lambda_res omegas gammas SZ] = ring_resonances_box(width, rOut, clambda, xs, nEffGuess, box_thickness)
tic
c = 299792458;           % m/s
...
for ii = 1:length(lambdas)
    %Set up index layers
    [nlyrs, dlyrsx, dlyrsy, left_to_rout] = eval([xs '(width,lambdas(ii), box_thickness)']);
